...
Interactive robot arm manipulation can be achieved in different ways. While end-to-end models still suffer from generalizability, hierarchical structure can be a good alternative, so that the manipulations remain robust, while perception can be generalized through vision-language model (VLM). Long horizon tasks can also be decomposed into smaller tasks, and the action sequence can be arranged by large language model (LLM).

In addition to the task pipeline, human robot interaction benefits a lot from the recent development of LLMs as well. It’s able to extract relevant information from voice command from the user. Since it’s already quite intelligent, we’re interested in incorporating other interaction approaches to the audio input, like human gaze and gestures, such that the robot system can understand human intention from combined interactions and react accordingly by executing various action sequences planned by LLM. Our vision is that such a system will enable the robot to be an assistant in household scenarios. 

---

## **2. Software Stack**
To enable natural human-robot interaction and efficient robot control, we designed a **software stack** that supports the entire system. The architecture consists of five main layers: **Interaction**, **Decision Making**, **Communication**, **Intention Modules**, and **Perception**.

### **2.1 Interaction**

At the interaction layer, we use **Whisper** as the speech interface. It processes natural voice commands from the user and converts them into text, enabling voice-driven interaction. This allows the user to control the robot directly through spoken instructions.
...
The communication layer translates high-level plans into low-level control commands for the robot. It includes:
...
With this layer, the abstract actions from the decision module can be executed seamlessly by the robot hardware.

### **2.4 Intention Modules**

The intention modules are designed to process non-verbal cues such as gestures and gaze:

- **Gesture (Mediapipe)**: detects and interprets hand and body gestures to complement voice input

- **Gaze (L2CS-Net)**: estimates the user’s gaze direction, helping the robot infer the user’s focus of attention

This allows the robot not only to “listen” to instructions but also to “read” the user’s intent.
...
- **OWL-ViT**: enables zero-shot object detection from textual descriptions, producing bounding boxes

- **SAM**(Segment Anything Model): generates segmentation masks for precise object recognition
...
#### **3.1.3 Grasp strategy**
The grasp strategy aims to automatically generate multiple candidate grasp poses from the object’s point cloud and then select the most reliable one through collision checking and containment evaluation. The overall workflow is illustrated in the figure:
...
<!-- <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:12px;">
  <iframe
    src="https://drive.google.com/file/d/1I53QuqV9eYbQR6_7gBvnvRwuM_uD_vWz/preview"
    allow="autoplay"
    style="position:absolute;top:0;left:0;width:90%;height:90%;border:0;"
  ></iframe>
</div> -->
...
[2]  Abdelrahman A A, Hempel T, Khalifa A, et al. L2cs-net: Fine-grained gaze estimation in unconstrained environments[C]//2023 8th International Conference on Frontiers of Signal Processing (ICFSP). IEEE, 2023: 98-102.

[3]  Heigold G, Minderer M, Gritsenko A, et al. Video owl-vit: Temporally-consistent open-world localization in video[C]//Proceedings of the IEEE/CVF International Conference on Computer Vision. 2023: 13802-13811.
...
[5] Zhao X, Ding W, An Y, et al. Fast segment anything[J]. arXiv preprint arXiv:2306.12156, 2023.
