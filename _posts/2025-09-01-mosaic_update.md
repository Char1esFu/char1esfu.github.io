...
- **Gaze (Aria Glass)** [6]: Gaze estimation is achieved by further development based on Meta's Aria glass, which can detect the user’s gaze direction, helping the robot infer the user’s focus of attention and further enhancing intention recognition.
...
- **YOLO** [7]: A model is trained for target objects of interest in our experiment, yields a bounding box and all the pixels within. 
...
<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/Interaction and decision-make framework update.png"
...
We’ve implemented high level and low level perception in our system. As part of the action type, perceive in high level perception framework is achieved by object detection with YOLO, extraction and filtering of point cloud within bounding box.
...
2.	**YOLO detection**: YOLO model infers the current image and outputs detected label(s) in real time. 

3.	**Decision branch — object found or not**:

      - An object list is given for YOLO model training. 

      - If the object is not found in the list, the system terminates the perception process and triggers TTS (Text-to-Speech) to provide user feedback, gracefully ending the action sequence.


      - If the object exists in the list, its bounding box and corresponding pixels will be parsed to the next module.


      - If zero shot detection is preferred, VLM can be used here to detect if the object is present in the image or not. 

4.	**Output to downstream modules**: As shown in Fig. 2, the prompt is then forwarded to the low-level perception module, where YOLO detection, 3D point clond extraction and filtering can take place.

<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/High-level perception framework update.png"
...
1. **3D mapping and point cloud filtering**: Using the intrinsic and extrinsic parameters of the camera, the parsed pixel coordinates are projected into 3D point clouds, effectively bridging the 2D image domain with the robot’s operational 3D workspace. Additionally a threshold along z-axis is applied to the point cloud, so that the points that represents the table can be filtered out. 

2. **Point cloud preprocessing**: To make the point cloud usable, several operations are applied:
...
3.	**Target point generation**: Finally, the system produces the target object’s point cloud along with a corresponding world-coordinate goal point, which serves as the foundation for grasping and downstream manipulation strategies.

<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/Low-level perception framework update.png"
       alt="Fig.3 Low-level perception framework"
       width="700">
  <figcaption><strong>Fig.3 Low-level perception framework</strong></figcaption>
</figure>

For the perception pipeline to work, we’ve taken photos for single objects as well as for multiple object combinations, augmented the raw dataset and trained the model with yolov11 [9]. The training results are shown as follows. The objects of interest are tested with high confidence score. If zero shot detection is desired, methods like OWL-ViT can be applied to high level perception.

<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/Validation results.jpg"
...
<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/Confusion matrix.png"
...
<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/F1 Confidence score.png"
...
The pitch and yaw angles of the gaze are estimated using a pretrained gaze model from Meta(https://github.com/facebookresearch/projectaria_eyetracking), providing the gaze direction in 3D space. An automatic calibration feature is also applied. It requires the user to look straight ahead. Pitch and yaw angles will then be calibrated to 0 degrees based on their average within the next 3s after calibration actuation. 
...
1. **Voice Node (Node 1)**:
This node continuously monitors spoken input and checks whether a new voice command has been issued. When a new command is detected, the node publishes both the transcription result and a Boolean flag indicating that a valid voice input is available. Otherwise, the Boolean flag remains False by default.

2. **Gesture Perception Node (Node 2)**:
This node subscribes to image and depth data to reconstruct the gesture array based on the fingertip position detected by MediaPipe. Using the reconstructed gesture direction, it computes the intersection point between the gesture vector and the table plane.

3. **Gaze Perception Node (Node 3)**:
This node performs glass localization using predefined markers and reconstructs the gaze array from pitch and yaw angles estimated by a pretrained model. Based on this gaze array direction, it computes the intersection point between the gaze vector and the table plane.

1. **Fusion and Intention Understanding Node (Node 4)**:
This node receives the voice transcription together with the gaze- and gesture-based labels. It first checks whether the current transcription is new. If a new voice command is detected, the node sends both the voice command and the perceptual labels to the Intention LLM, which performs a comprehensive analysis and outputs the final intention command. If no new transcription is available, the system remains inactive.

Before sending the perceptual labels to the fusion node, the system evaluates whether the intersection between gaze and gesture is stable, i.e., whether it does not wobble significantly over time. If the intersection is stable, YOLO is applied to the regions of interest (ROIs) around the intersection point, and the object labels from the two ROIs are published separately. Otherwise, None is published.
...
By separately processing voice, gesture, and gaze information and then fusing them at the intention understanding stage, the updated module improves the robustness and naturalness of intention recognition, thereby enhancing the reliability of human–robot interaction.

<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/Multimodal intent fusion process update.png"
...
<div style="max-width:700px;margin:0 auto;position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:12px;">
  <iframe
    src="https://drive.google.com/file/d/1J9MrSA66BdNE-sFbmopqq2KmS6iZBD5v/preview"
    allow="autoplay"
...
<div style="max-width:700px;margin:0 auto;position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:12px;">
  <iframe
    src="https://drive.google.com/file/d/1AgKgixZegHq87KB1d3JPoE69amZy3wz-/preview"
    allow="autoplay"
    style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;"
    allowfullscreen
  ></iframe>
...
<div style="max-width:700px;margin:0 auto;position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:12px;">
  <iframe
    src="https://drive.google.com/file/d/1pVwWwW662VB0Wrflu0Zfu4OLYjjpkWzc/preview"
    allow="autoplay"
    style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;"
    allowfullscreen
  ></iframe>
...
<div style="max-width:700px;margin:0 auto;position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:12px;">
  <iframe
    src="https://drive.google.com/file/d/15IuDlgjv1biang1Hc3m4tJol8po978KV/preview"
    allow="autoplay"
    style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;"
    allowfullscreen
  ></iframe>
...
<figure style="text-align: center;">
  <img src="/assets/mosaic_imgs/scene setup.jpg"
       alt="Fig.12 scene setup"
       width="700">
  <figcaption><strong>Fig.12 scene setup</strong></figcaption>
</figure>

In this project, we designed and implemented a modular system for assistive and interactive cooking. By combining YOLO detection, large language models (LLMs), and multimodal interaction part (voice command, gaze and gesture), the system enables natural and flexible collaboration between humans and robots. It can understand spoken instructions, perceive the environment, and infer user intentions even without explicit text input, while executing complex cooking-related tasks through a hierarchical control pipeline. With a multi-camera setup and a library of fundamental robotic skills, the system demonstrates how generalization and context-aware interaction can be achieved in human-robot collaboration.
...
Looking ahead, there are several promising directions to extend this work. First, incorporating dynamic obstacle avoidance will improve safety in cluttered and changing environments. Second, adopting learning-based grasping strategies such as VGN, GPD, GIGA can replace handcrafted heuristics, enabling more robust manipulation of diverse kitchen objects. Third, deploying LLMs locally will reduce latency, making the system more efficient and practical for real-world use. Finally, further VRAM optimization can broaden hardware compatibility and enhance responsiveness.
...
[6] Abdelrahman A A, Hempel T, Khalifa A, et al. L2cs-net: Fine-grained gaze estimation in unconstrained environments[C]//2023 8th International Conference on Frontiers of Signal Processing (ICFSP). IEEE, 2023: 98-102.
